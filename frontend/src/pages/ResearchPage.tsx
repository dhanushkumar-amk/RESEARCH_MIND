                  ...
                  {availableDocs.map(doc => (
                    ...
                    >
                      <input
                        ...
                        className="rounded border-neutral-350 text-emerald-600 focus:ring-emerald-500/10 h-3.5 w-3.5 cursor-pointer"
                      />
            ...
            <div className="space-y-1 overflow-y-auto flex-1">
              {messages.filter(m => m.role === 'user').map((m, idx) => (
                <button
                  ...
                  className="w-full text-left text-xs text-neutral-600 hover:text-neutral-900 hover:bg-neutral-50 p-2 rounded-lg truncate block font-medium"
                >
                  {m.content}
                </button>
              ))}
              {messages.filter(m => m.role === 'user').length === 0 && (
                <div className="text-center py-8 text-neutral-405 text-xs">
        ...
        <div className="lg:col-span-3 flex flex-col bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.02)] h-full">
          ...
          <div className="bg-neutral-50/50 border-b border-neutral-100 p-3.5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded transition-all uppercase ${
                activeAgent === 'summary' ? 'bg-emerald-100 text-emerald-700 animate-pulse' : 'bg-neutral-100 text-neutral-400'
              }`}>
                Summary
              </span>
            </div>
                ...
                <div key={index} className="space-y-2">
                  ...
                  {message.role === 'user' && (
                    <div className="flex justify-end">
                      <div className="bg-neutral-100 text-neutral-900 rounded-2xl rounded-tr-sm px-4 py-2.5 max-w-[80%] text-sm font-medium">
                        {message.content}
                      </div>
                    </div>
                  )}
                  ...
                  {message.role === 'assistant' && (
                    <div className="space-y-4 max-w-[90%]">
                      ...
                      {/* Main response body */}
                      <div className="prose prose-neutral text-sm text-neutral-850 leading-relaxed bg-white border border-neutral-150 rounded-2xl rounded-tl-sm p-4 shadow-sm whitespace-pre-line">
                        {message.content}
                        {message.isStreaming && (
                          ...
                          <div className="flex flex-wrap gap-2">
                            {message.sources.map(src => (
                              <a 
                                key={src.id}
                                href="#"
                                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-neutral-50 border border-neutral-200 text-neutral-700 hover:border-emerald-200 hover:text-emerald-700 transition-colors text-xs font-semibold"
                              >
                                {src.type === 'PDF' ? <BookOpen className="h-3 w-3 text-rose-500" /> : <Globe className="h-3 w-3 text-emerald-500" />}
                      ...
                      {!message.isStreaming && (
                        <div className="flex items-center justify-between border-t border-neutral-100 pt-3">
                          ...
                          <div className="flex items-center gap-2">
                            <button className="p-1.5 text-neutral-400 hover:text-emerald-600 rounded-md hover:bg-neutral-50 transition-all" title="Thumbs Up">
                          ...
                          <button 
                            ...
                            className="bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all shadow-sm cursor-pointer"
                          >
                      ...
                      {!message.isStreaming && message.suggestions && (
                        <div className="space-y-2 pt-2 border-t border-dashed border-neutral-100">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            {message.suggestions.map(sug => (
                              <button
                                ...
                                className="text-left text-xs text-neutral-700 bg-neutral-50 hover:bg-emerald-50/50 hover:text-emerald-800 p-2.5 rounded-xl border border-neutral-150 transition-all font-medium flex items-center justify-between group"
                              >
                                <span className="truncate">{sug.question}</span>
                                <ChevronRight className="h-3.5 w-3.5 text-neutral-400 group-hover:text-emerald-500 group-hover:translate-x-0.5 transition-all flex-shrink-0" />
                              </button>
                            ))}
                          </div>
                        </div>
            ...
            >
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full pl-4 pr-12 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 bg-white transition-all text-neutral-900 placeholder-neutral-400 shadow-inner disabled:bg-neutral-100"
                />
              ...
              <button
                ...
                className="bg-neutral-950 text-white hover:bg-neutral-850 disabled:bg-neutral-100 disabled:text-neutral-400 font-semibold p-3 rounded-xl transition-all shadow-sm flex items-center justify-center cursor-pointer"
              >
